 ...
 *
 * This script processes ONLY documents with name === 'Stylized Kitchen' (case-insensitive).
 */
...
// main migration
async function migrateStylizedKitchen() {
  console.log('🔁 Starting Ambients (Stylized Kitchen) migration...');
  await startDB();

  // Find documents where name is Stylized Kitchen (case-insensitive)
  const docs = await AmbientModel.find({ name: { $regex: '^\\s*Stylized Kitchen\\s*$', $options: 'i' } });

  if (!docs || docs.length === 0) {
    console.log('ℹ No "Stylized Kitchen" documents found. Exiting.');
    process.exit(0);
  }
  ...
  console.log('\n🎉 Migration for Stylized Kitchen complete.');
  process.exit(0);
}

// run when executed directly
if (require.main === module) {
  migrateStylizedKitchen().catch(err => {
...
module.exports = migrateStylizedKitchen;
