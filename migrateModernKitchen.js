 ...
 *
 * This script processes ONLY documents with name === 'Modern Kitchen' (case-insensitive).
 */
...
// main migration
async function migrateModernKitchen() {
  console.log('🔁 Starting Ambients (Modern Kitchen) migration...');
  await startDB();

  // Find documents where name is Modern Kitchen (case-insensitive)
  const docs = await AmbientModel.find({ name: { $regex: '^\\s*Modern Kitchen\\s*$', $options: 'i' } });

  if (!docs || docs.length === 0) {
    console.log('ℹ No "Modern Kitchen" documents found. Exiting.');
    process.exit(0);
  }
  ...
  console.log('\n🎉 Migration for Modern Kitchen complete.');
  process.exit(0);
}
...
module.exports = migrateModernKitchen;
