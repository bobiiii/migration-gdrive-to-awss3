 ...
 *
 * This script processes ONLY documents with name === 'Modern Bathroom' (case-insensitive).
 */
...
// main migration
async function migrateModernBathroom() {
  console.log('🔁 Starting Ambients (Modern Bathroom) migration...');
  await startDB();

  // Find documents where name is Modern Bathroom (case-insensitive)
  const docs = await AmbientModel.find({ name: { $regex: '^\\s*Modern Bathroom\\s*$', $options: 'i' } });

  if (!docs || docs.length === 0) {
    console.log('ℹ No "Modern Bathroom" documents found. Exiting.');
    process.exit(0);
  }
  ...
  console.log('\n🎉 Migration for Modern Bathroom complete.');
  process.exit(0);
}
...
module.exports = migrateModernBathroom;
